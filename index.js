...
✅ Отзывы наших клиентов - https://vk.com/yogoshop`, { keyboard: KeybroadPrivate(_users) })
})
...
if(su.text == 'Подтвердить'){
	...
	let aqq = 0
    for(i in a){
		vk.api.messages.send({ message: `${sum.text} \n\n❌ Чтобы отписаться от рассылки и не получать сообщения от бота, нажмите кнопку "Отписаться".`, attachment: sus.text , user_id: a[i].id, random_id: Math.random*999, keyboard: Keyboard.keyboard([ Keyboard.urlButton({ url: "https://vk.com/yogoshop", label: "Отзывы", color: Keyboard.POSITIVE_COLOR }), Keyboard.textButton({ label: "Отписаться", color: Keyboard.NEGATIVE_COLOR }) ]).inline(true) })
	    aqq += 1
	}
...
vk.updates.on('new_wall_repost', async (context) => {
  ...
  let a = await vk.api.groups.isMember({ group_id: 215562047, user_id: context.wall.ownerId })
  if(a == 0){
...
💬 Пожалуйста, оставьте отзыв, чтобы другие люди поверили в нашу честность: https://vk.com/yogoshop`,
	...
	response.map(async (operation) => {
		...
		if(!user) return;
			...
			try {
				...
				await vk.api.messages.send({ peer_id: 2, message: `🔥 *id${us.id} (${us.first_name}) перевёл ${utils.split(rubAmount)}₽ \n✅ Ему успешно отправлено ${utils.split(amount)} VkCoin` }).catch((err) => { return console.log(`Ошибка при отправке сообщения!`) })
				await vk.api.messages.send({
					user_id: id,
				message: `✅ Поступил платёж в размере ${utils.split(rubAmount)}₽ 
💰 Вам отправлено ${utils.split(amount)} VKCoin

💬 Пожалуйста, оставьте отзыв, чтобы другие люди поверили в нашу честность: https://vk.com/yogoshop` }).catch((err) => { return console.log(`Ошибка при отправке сообщения!`) })
				} catch (e) {
				await vk.api.messages.send({
					user_id: id,
